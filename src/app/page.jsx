...
export default function Portfolio(){
  ...
  return <main className={styles.page}>
    ...
    <section id="skills" className={styles.skills}><div className={styles.sectionHead}><div><p className={styles.eyebrow}>03 — TOOLKIT</p><h2>The stack behind<br/><em>the work.</em></h2></div><p>Technologies I use across web, mobile, backend and connected applications.</p></div><div className={styles.stackCircle}><svg className={styles.stackConnectors} viewBox="0 0 1000 620" preserveAspectRatio="none" aria-hidden="true"><circle cx="500" cy="310" r="126"/>{[[220,110,420,250],[500,70,500,184],[780,110,580,250],[790,480,600,360],[500,550,500,436],[210,480,400,360]].map(([x1,y1,x2,y2],i)=><line key={i} x1={x1} y1={y1} x2={x2} y2={y2}/>)}</svg><div className={styles.stackRing}><div className={styles.stackRingInner}><b>BH</b><span>BUILD · INNOVATE · DEPLOY</span><small>FULL STACK ENGINEER</small></div></div>{skillGroups.map((g,i)=><article key={g[0]} className={styles["stackBox"+i]}><div><small>0{i+1}</small><b>{g[0]}</b></div><div className={styles.stackItems}>{g.slice(1).map(v=><span key={v}>{v}</span>)}</div><footer><span>{i===0?"LANGUAGES":i===1?"WEB":i===2?"MOBILE":i===3?"BACKEND":i===4?"DATA":"DEVOPS & TOOLS"}</span><i>↗</i></footer></article>)}</div></section>
