...
function Section({ id, eyebrow, title, children }) {
  return <Box component="section" id={id} sx={sectionSx}><Container maxWidth="lg">
    <Stack spacing={1} sx={{ mb: 4 }}><Typography sx={{ color: "#818cf8", fontWeight: 800, fontSize: ".78rem", letterSpacing: 2, textTransform: "uppercase" }}>{eyebrow}</Typography><Typography component="h2" sx={{ fontWeight: 850, fontSize: { xs: "2rem", md: "3rem" }, letterSpacing: -.8, color: "#f8fafc" }}>{title}</Typography></Stack>{children}
  </Container></Box>
}
...
function ProjectCard({ project }) {
  ...
  return <Card sx={{ height: "100%", background: "linear-gradient(145deg, rgba(17,24,39,.92), rgba(10,15,28,.94))", border: "1px solid rgba(148,163,184,.14)", borderRadius: 4, transition: "transform .25s ease, border-color .25s ease", "&:hover": { transform: "translateY(-5px)", borderColor: "rgba(129,140,248,.45)" } }}><CardContent sx={{ p: { xs: 3, md: 3.5 }, height: "100%", display: "flex", flexDirection: "column" }}>
    <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}><Chip label={project.status} size="small" sx={{ ...chipSx, border: "1px solid", borderColor: project.status === "In progress" ? "rgba(251,191,36,.35)" : "rgba(129,140,248,.3)", color: project.status === "In progress" ? "#fbbf24" : "#a5b4fc" }} /><Typography sx={{ color: "#64748b", fontSize: ".75rem", fontWeight: 700 }}>PROJECT</Typography></Stack>
    <Typography component="h3" sx={{ fontSize: "1.35rem", fontWeight: 800, mb: 1.5, color: isSaas ? "#a5b4fc" : "#f8fafc", textShadow: isSaas ? "0 0 18px rgba(129,140,248,.28)" : "none" }}>{project.title}</Typography><Typography sx={{ color: "#94a3b8", lineHeight: 1.75, fontSize: ".95rem", mb: 2.5 }}>{project.description}</Typography>
  ...
  return <Box sx={{ minHeight: "100vh", background: "#070a12", color: "#f8fafc", overflow: "hidden", "& a": { textDecoration: "none" } }}>
    ...
    <Box id="top" component="main" sx={{ position: "relative", zIndex: 1 }}>
      <Container maxWidth="lg" sx={{ pt: { xs: 10, md: 15 }, pb: { xs: 8, md: 12 } }}><Box sx={{ maxWidth: 930 }}>
        ...
        <Typography component="h1" sx={{ fontSize: { xs: "3.5rem", sm: "5.2rem", md: "7rem" }, lineHeight: .95, letterSpacing: -5, fontWeight: 900, color: "#f8fafc" }}>Bhargav <Box component="span" sx={{ color: "#818cf8" }}>HJ</Box></Typography>
      ...
      <Section id="skills" eyebrow="Toolkit" title="Technologies I work with"><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2,1fr)", lg: "repeat(3,1fr)" }, gap: 2 }}>{skills.map((group) => <Card key={group.title} sx={{ background: "rgba(15,23,42,.58)", border: "1px solid rgba(148,163,184,.11)", borderRadius: 3 }}><CardContent sx={{ p: 3 }}><Typography component="h3" sx={{ fontWeight: 800, mb: 2, color: "#f8fafc" }}>{group.title}</Typography><Stack direction="row" spacing={.8} useFlexGap flexWrap="wrap">{group.items.map((item) => <Chip key={item} label={item} size="small" sx={chipSx} />)}</Stack></CardContent></Card>)}</Box></Section>
      ...
      <Section id="experience" eyebrow="Professional experience" title="Building production software"><Card sx={{ background: "linear-gradient(145deg, rgba(17,24,39,.95), rgba(10,15,28,.95))", border: "1px solid rgba(129,140,248,.16)", borderRadius: 4 }}><CardContent sx={{ p: { xs: 3, md: 5 } }}>
        <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" spacing={2}><Box><Typography component="h3" sx={{ fontSize: "1.6rem", fontWeight: 850, color: "#f8fafc" }}>Full Stack Developer</Typography><Typography sx={{ color: "#a5b4fc", mt: .5, fontWeight: 700 }}>Skoegle IoT Innovations Pvt. Ltd.</Typography></Box><Chip label="Oct 2024 – Present" sx={chipSx} /></Stack>
      ...
      <Typography component="h3" sx={{ fontWeight: 800, fontSize: "1.35rem", mt: 6, mb: 2.5, color: "#f8fafc" }}>Selected professional projects</Typography><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3,1fr)" }, gap: 2 }}>{professionalProjects.map((project) => <Card key={project.title} sx={{ background: "rgba(15,23,42,.55)", border: "1px solid rgba(148,163,184,.11)", borderRadius: 3 }}><CardContent sx={{ p: 3 }}><Typography component="h4" sx={{ fontWeight: 800, mb: 1.5, color: "#f8fafc" }}>{project.title}</Typography><Typography sx={{ color: "#94a3b8", lineHeight: 1.7, fontSize: ".9rem", mb: 2 }}>{project.description}</Typography><Stack direction="row" spacing={.7} useFlexGap flexWrap="wrap">{project.technologies.map((item) => <Chip key={item} label={item} size="small" sx={chipSx} />)}</Stack></CardContent></Card>)}</Box></Section>

      <Section id="education" eyebrow="Education" title="Academic foundation"><Card sx={{ background: "rgba(15,23,42,.55)", border: "1px solid rgba(148,163,184,.11)", borderRadius: 3 }}><CardContent sx={{ p: 3.5 }}><Stack direction="row" spacing={2} alignItems="flex-start"><Box sx={{ width: 44, height: 44, display: "grid", placeItems: "center", borderRadius: 2, background: "rgba(129,140,248,.10)" }}><School sx={{ color: "#818cf8" }} /></Box><Box><Typography component="h3" sx={{ fontWeight: 800, color: "#f8fafc" }}>Bachelor of Engineering — Information Science</Typography><Typography sx={{ color: "#94a3b8", mt: .5 }}>Dr. Ambedkar Institute of Technology, Bangalore · 2023</Typography><Typography sx={{ color: "#64748b", mt: .5 }}>CGPA: 6.5 / 10</Typography></Box></Stack></CardContent></Card></Section>
